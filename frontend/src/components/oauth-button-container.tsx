...
export default function OAuthButtonContainer() {
  const providers = [
    { name: "Google", icon: <GoogleIcon />, comingSoon: false },
    { name: "GitHub", icon: <GithubIcon />, comingSoon: false },
    { name: "LinkedIn", icon: <LinkedinIcon />, comingSoon: false },
  ];

  return (
    <div className="space-y-3">
      {providers.map(({ name, icon, comingSoon }) => (
        <OAuthButton key={name} icon={icon} provider={name} comingSoon={comingSoon} />
      ))}
    </div>
  );
}
