...
Schema includes:
- 4 user tables: users, oauth_accounts, deleted_accounts
  (hard-delete audit log)
- 4 course tables: courses, sections, enrollments, user_progress
  (section-based progress)
- 5 quiz tables: quizzes, questions, quiz_attempts,
  quiz_attempt_answers
- All tables use UUID primary keys with gen_random_uuid()
- Proper foreign key constraints with CASCADE delete
- Critical constraint: section_id_str UNIQUE (preserves user
  progress across file renames)
- OAuth security: NO access_token stored, refresh_token AES-256
  encrypted
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.String(20),
            nullable=False,
            server_default=sa.text("'initials'"),
        ),
        # avatar_type: 'initials' (default) | 'system_1' ... 'system_10'
        # Frontend generates initials from display_name OR displays
        # /avatars/avatar_{n}.svg
    ...
    op.create_table(
        ...
        # refresh_token_encrypted: AES-256 encrypted IF provider
        # supplies it
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.String(50),
            nullable=True,
            server_default=sa.text("'user_request'"),
        ),
        sa.PrimaryKeyConstraint('id'),
    )
    ...
    op.create_table(
        ...
        # category: 'programlama' | 'web' | 'veri' | etc.
        # (used for filtering on FR-07)
        sa.Column('difficulty', sa.String(20), nullable=True),
        # difficulty: 'beginner' | 'intermediate' | 'advanced'
        # (filtering on FR-07)
        sa.Column('duration_minutes', sa.Integer(), nullable=True),
        # duration_minutes: Seeded from meta.json.
        # Displayed on course card (FR-08)
        sa.Column(
            'is_published',
            sa.Boolean(),
            nullable=False,
    ...
    op.create_table(
        ...
        # section_id_str: Unique identifier from MDX frontmatter
        # (e.g. "python-001-giris")
        # CRITICAL: This ID is PERMANENT. If file renamed, ID never changes.
        # Used by user_progress to link section completion across schema
        # evolution.
        sa.Column('title', sa.String(255), nullable=False),
        ...
        # Allows linking user_progress even if file moves between courses
        ...
        sa.UniqueConstraint(
            'course_id', 'order_index', name='uq_sections_course_order'
        ),
    )
    ...
    op.create_table(
        ...
        # completed_at: Set when ALL sections marked completed
        # (no partial completion)
        # Requirement FR-13: Kurs "tamamlandı" = tüm section'lar
        # tamamlandı
        sa.Column(
            'progress_percent',
            sa.Numeric(5, 2),
    ...
    op.create_table(
        ...
        # (dashboard query: count TRUE sections)
        sa.Column('completed_at', sa.DateTime(timezone=True), nullable=True),
        ...
        sa.UniqueConstraint(
            'user_id', 'section_id', name='uq_progress_user_section'
        ),
    )
    op.create_index(
    ...
    op.create_table(
        ...
        # duration_seconds: Quiz time limit in seconds
    ...
    op.create_table(
        ...
        sa.Column(
            'options', postgresql.JSONB(astext_type=sa.String()), nullable=False
        ),
        # options: JSON array [{"index": 0, "text": "Answer A"},
        # {"index": 1, "text": "Answer B"}, ...]
        sa.Column('correct_index', sa.Integer(), nullable=False),
        # correct_index: Index of correct answer (0-based).
        # NEVER sent to client before submit.
        # Requirement NF-05: Quiz correct_index client'a gönderilmez
        # (submit öncesi)
        sa.Column('explanation', sa.Text(), nullable=True),
        # explanation: Why this answer is correct.
        # Shown on result review page (FR-18)
        sa.Column('order_index', sa.Integer(), nullable=False),
    ...
    op.create_table(
        ...
        # Used for time_spent calculation.
        # Security: Backend checks submitted_at - started_at <=
        # duration_seconds + 30s (tolerance)
        ...
        # time_spent_secs: submitted_at - started_at in seconds
        # (shown on result card)
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['quiz_id'], ['quizzes.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id'),
    )
    op.create_index(
        'idx_attempts_user_quiz', 'quiz_attempts', ['user_id', 'quiz_id']
    )
    ...
    op.create_table(
        ...
        # selected_index: Index of selected answer (0-based),
        # NULL = unanswered (time limit reached)
        # Requirement: Support unanswered questions when time limit
        # expires before answer submission
        sa.Column(
...
def downgrade() -> None:
    ...
    (bottom-up to respect FK constraints)
    """
