...
async def test_auth_rate_limit_exceeded():
    ...
    async with AsyncClient(
        ...
        r = await ac.post("/auth/login", headers={"X-Forwarded-For": "203.0.113.1"})
...
async def test_api_rate_limit_exceeded():
    ...
    async with AsyncClient(
        ...
        for i in range(100):
            r = await ac.get("/api/users", headers={"X-Forwarded-For": "203.0.113.2"})
            assert r.status_code != 429, f"Request {i + 1} should not be rate limited"
...
async def test_retry_after_seconds_positive():
    ...
    async with AsyncClient(
        ...
        # Limit'e ulaş
        for _ in range(10):
            await ac.post("/auth/login", headers={"X-Forwarded-For": "203.0.113.4"})

        # 11. istek
        r = await ac.post("/auth/login", headers={"X-Forwarded-For": "203.0.113.4"})
...
async def test_retry_after_header_present():
    ...
    async with AsyncClient(
        ...
        # Limit'e ulaş
        for _ in range(10):
            await ac.post("/auth/login", headers={"X-Forwarded-For": "203.0.113.5"})

        # 11. istek - 429 al
        r = await ac.post("/auth/login", headers={"X-Forwarded-For": "203.0.113.5"})
...
async def test_different_ips_independent_counters():
    ...
    async with AsyncClient(
        ...
        # IP 1: Limit doldur
        for _ in range(10):
            await ac.post("/auth/login", headers={"X-Forwarded-For": "1.1.1.1"})

        # IP 1: 11. istek - 429
        r = await ac.post("/auth/login", headers={"X-Forwarded-For": "1.1.1.1"})
        assert r.status_code == 429, "IP 1.1.1.1 should be rate limited"

        # IP 2: İlk istek - geçmeli
        r = await ac.post("/auth/login", headers={"X-Forwarded-For": "2.2.2.2"})
...
async def test_independent_endpoint_patterns():
    ...
    async with AsyncClient(
        ...
        # Auth limit doldur
        for _ in range(10):
            await ac.post("/auth/login", headers={"X-Forwarded-For": "203.0.113.6"})

        # Auth rate limited
        r = await ac.post("/auth/login", headers={"X-Forwarded-For": "203.0.113.6"})
        assert r.status_code == 429, "Auth should be rate limited"

        # API hala çalışmalı (aynı IP!)
        r = await ac.get("/api/users", headers={"X-Forwarded-For": "203.0.113.6"})
        assert r.status_code != 429, (
...
async def test_options_preflight_exempt():
    ...
    async with AsyncClient(
        ...
        for _ in range(150):
            r = await ac.options(
                "/auth/login", headers={"X-Forwarded-For": "203.0.113.7"}
            )
        ...
        r = await ac.post("/auth/login", headers={"X-Forwarded-For": "203.0.113.7"})
