...
class DashboardService:
    ...
    def get_summary(db: Session, user_id: Any) -> dict:
        ...
        completed_count = (
            db.query(Enrollment)
            .filter(
                Enrollment.user_id == user_uuid, Enrollment.completed_at.isnot(None)
            )
            .count()
        )
        ...
        enrollments = (
            db.query(Enrollment)
            .filter(Enrollment.user_id == user_uuid, Enrollment.completed_at.is_(None))
            .all()
        )
        ...
        if enrollments:
            ...
            completed_sections_ids = (
                db.query(UserProgress.section_id)
                .filter(
                    UserProgress.user_id == user_uuid, UserProgress.completed
                )
                .subquery()
            )

            # Her enrollment için sonraki tamamlanmamış bölümü bul 
            # (en düşük order_index)
        ...
        last_attempt = (
            ...
            .join(QuizAttempt.quiz.course)
            .filter(
                QuizAttempt.user_id == user_uuid, QuizAttempt.submitted_at.isnot(None)
            )
        ...
        if last_attempt:
            last_quiz_data = {
                # Quiz adı olarak kurs başlığını kullan
