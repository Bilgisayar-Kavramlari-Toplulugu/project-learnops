...
class DashboardService:
    ...
    async def get_summary(db: AsyncSession, user_id: UUID) -> Dict[str, Any]:
        ...
        1. Subquery: Tamamlanmış bölümler Python belleğine çekilmeden DB içinde filtreleme için kullanıldı.
        2. Bulk Next Section: N+1 problemini çözmek için döngü içindeki sorgular tek bir 'DISTINCT ON' 
           benzeri yapıya (veya tek sorguya) indirgendi.
        ...
        if enrollments:
            ...
            # DISTINCT ON (course_id) mantığı SQLAlchemy .distinct() ile uygulanır.
            next_sections_stmt = (
                select(Section)
                .where(
